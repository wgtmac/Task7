	...
	public String perform(HttpServletRequest request) {
		...
		try {
			...
			if (session.getAttribute("user") != null
				...
				request.setAttribute("customerName", customer.getFirstName()
				...
				request.setAttribute("lastDay",
						lastDay == null ? "No recent transactions" : lastDay);

				request.setAttribute("cash",
						df2.format(customer.getCash() / 100.0));
				request.setAttribute("avai_cash", df2.format(transactionDAO
						.getValidBalance(customer.getUserName(),
								customer.getCash() / 100.0)));

				PositionBean[] fundList = positionDAO.getFunds(form
						.getUserName1());
				request.setAttribute("fundList", fundList);

				List<PositionBean> positionList = positionDAO
						.getAllPositionByCustomer(customer);
				if (positionList != null) {
					...
					for (PositionBean a : positionList) {
						...
						double price = ((double) (historyDAO
								.getLatestFundPrice(a.getFundId()).getPrice() / 100.0));
						...
						PositionInfo aInfo = new PositionInfo(ticker,
								sharesString, priceString, "$" + valueString);
